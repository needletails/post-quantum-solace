...
public extension PQSSession {
    ...
    ///   - `PQSError.sessionNotInitialized` if the session is not properly initialized
    ///   - `PQSError.invalidSignature` if cryptographic operations fail
    ///
    ...
    ///   - `PQSError.sessionNotInitialized` if the session is not properly initialized
    ///   - `PQSError.databaseNotInitialized` if the cache is not available
    ///   - `PQSError.transportNotInitialized` if the transport delegate is not set
    ///   - `PQSError.invalidSignature` if device verification fails
    ///   - `PQSError.drainedKeys` if one-time keys are not available
    ///
    ...
    func sessionIdentitiesForChatFanout(secretName: String) async throws -> [SessionIdentity] {
        ...
        if offlineOutboundPersist {
            ...
            if verifiedIds.isEmpty {
                throw PQSError.cannotFindUserConfiguration
            }
        ...
        for deviceId in verifiedIds {
            guard let identity = await messagePipeline.outboundSessionIdentity(
                secretName: secretName,
                deviceId: deviceId,
                in: refreshed,
        ...
        if byDevice.isEmpty, !verifiedIds.isEmpty, isViable {
            ...
            if !byDevice.isEmpty {
                auditSink.log(.recovery, "pqs.recovery.outboundLaneRepairedOnDemand peer=\(secretName) repairedDevices=\(byDevice.count)")
            }
        }
    ...
    /// device (`props.hasRatchetState`). A session-identity row alone is not enough: the
    ...
    internal func deliverPeerHandshakeNotifyBeforeOutboundSenderInit(secretName: String) async throws -> Bool {
        ...
        if props.oneTimePublicKey == nil {
            ...
            try await identity.update(props, symmetricKey: symmetricKey)
            try await cache?.updateSessionIdentity(identity)
        }

        guard let recipientX25519Id = props.oneTimePublicKey?.id.uuidString else {
            logger.log(
                level: .warning,
    ...
    internal func peerHasPublishedX25519OneTimeKey(
        secretName: String,
        deviceId: UUID
    ) async throws -> Bool {
        guard let transportDelegate else { return false }
        let x25519Ids = try await transportDelegate.fetchOneTimeKeyIdentities(
            for: secretName,
            deviceId: deviceId.uuidString,
            type: .x25519)
        return !x25519Ids.isEmpty
    }

    /// True when the peer has a published curve OTK that can be verified from live configuration.
    internal func peerCanSupplyX25519OneTimeKey(
        secretName: String,
        deviceId: UUID
    ) async throws -> Bool {
        guard let transportDelegate else { return false }
        ...
        let x25519Ids = try await transportDelegate.fetchOneTimeKeyIdentities(
            ...
            deviceId: deviceId.uuidString,
            type: .x25519)
        return try resolvePublishedX25519OneTimeKey(
            x25519Ids: x25519Ids,
            configuration: configuration,
            device: device) != nil
    }
    ...
    internal func peerHasEncryptableSessionForControl(
        ...
        for identity in identities {
            guard let props = await identity.props(symmetricKey: symmetricKey) else { continue }
            guard props.deviceId == deviceId, props.hasRatchetState else { continue }
            return true
        }
        return false
    }
    ...
    func wipePeerRelationshipState(secretName: String) async throws {
        ...
        auditSink.log(.recovery, "pqs.recovery.laneWiped peer=\(secretName) deletedRows=\(deletedCount)")
    }
    ...
    internal func awaitPeerReplenishCompletion(
        ...
        if (try? await peerCanSupplyX25519OneTimeKey(secretName: secretName, deviceId: deviceId)) == true {
            return true
        }
        ...
        return (try? await peerCanSupplyX25519OneTimeKey(secretName: secretName, deviceId: deviceId)) == true
    }
    ...
    internal func repairPeerPublishedOneTimeKeysIfPossible(
        ...
        guard try await !peerCanSupplyX25519OneTimeKey(secretName: secretName, deviceId: deviceId) else {
            return
        }
    ...
    internal func attachPublishedPeerOneTimeKeys(
        ...
        let x25519Ids = try await transportDelegate.fetchOneTimeKeyIdentities(
            ...
            type: .x25519)
        if let signedX25519 = try resolvePublishedX25519OneTimeKey(
            x25519Ids: x25519Ids,
            configuration: configuration,
            device: device) {
            props.oneTimePublicKey = signedX25519
        } else {
    ...
    private func resolvePublishedX25519OneTimeKey(
        x25519Ids: [UUID],
        configuration: UserConfiguration,
        device: UserDeviceConfiguration
    ) throws -> X25519PublicKey? {
        let deviceSigningPublicKey = try Curve25519.Signing.PublicKey(rawRepresentation: device.signingPublicKey)
        for x25519Id in x25519Ids.reversed() {
            if let signedX25519 = try configuration.signedOneTimePublicKeys
                .first(where: { $0.id == x25519Id })?
    ...
    /// (`respondToSession` → `setState`) only runs when the snapshot is absent.
    ...
    internal func activateSessionIdentityAfterInboundDecrypt(
        ...
        if demotedSiblingCount > 0 {
            ...
            auditSink.log(.recovery, "pqs.recovery.laneActivatedAfterDecrypt peer=\(provenProps.secretName) deviceId=\(provenProps.deviceId.uuidString) demotedSiblings=\(demotedSiblingCount)")
        }
    ...
    internal func demoteActiveSessionIdentityToInactive(
        ...
        let symmetricKey = try await getDatabaseSymmetricKey()
        guard var props = await identity.props(symmetricKey: symmetricKey),
              !props.deviceName.hasPrefix(PQSSessionConstants.inactiveSessionDeviceNamePrefix)
        else {
            return false
        }
        ...
        props.sessionContextId = Int(Date().timeIntervalSince1970)
        props.deviceName = PQSSessionConstants.inactiveSessionDeviceNamePrefix + props.deviceName
        try await identity.update(props, symmetricKey: symmetricKey)
        try await cache.updateSessionIdentity(identity)
    ...
    internal func demoteProveFailedActive(
        ...
        if demoted {
            invalidateSessionIdentityCache(secretName: secretName)
            auditSink.log(.recovery, "pqs.recovery.proveFailedActiveDemoted peer=\(secretName) deviceId=\(deviceId.uuidString) sessionId=\(sessionId.uuidString)")
            logger.log(
                level: .warning,
    ...
    internal func demoteZombieStateLessActives(
        ...
        if demoted > 0 {
            ...
            invalidateSessionIdentityCache(secretName: secretName)
            auditSink.log(.recovery, "pqs.recovery.zombieStateLessDemoted peer=\(secretName) deviceId=\(deviceId.uuidString) demoted=\(demoted)")
            logger.log(
                level: .warning,
    ...
    internal func promoteArchivedSessionIdentityToActive(
        ...
        var restoredProps = archivedProps
        restoredProps.deviceName = restoredDeviceName
        try await archived.update(restoredProps, symmetricKey: symmetricKey)
        ...
        auditSink.log(.recovery, "pqs.recovery.lanePromotedFromArchive peer=\(archivedProps.secretName) deviceId=\(archivedProps.deviceId.uuidString) demotedActive=\(demotedActiveCount)")
        return archived
    }
    ...
    internal func resetSessionIdentityForFreshSession(
        ...
        if !sendOneTimeIdentities,
           ...
           activeMatches.allSatisfy({ !$0.props.hasRatchetState }) {
            ...
            if keysMatchDevice {
                ...
                auditSink.log(.recovery, "pqs.recovery.laneReset outcome=reusedStateLessRow reason=\(reason) peer=\(secretName) deviceId=\(device.deviceId.uuidString)")
        ...
        let (x25519, mlKEM) = try await createOneTimeKeys(
            secretName: secretName,
            deviceId: device.deviceId,
            x25519Id: nil,
            mlKEMId: nil,
        ...
        if demotePriorActives {
            auditSink.log(.recovery, "pqs.recovery.laneReset outcome=reset reason=\(reason) peer=\(secretName) deviceId=\(device.deviceId.uuidString) demotedActive=\(demotedActiveCount) consumedOTK=\(sendOneTimeIdentities)")
        } else {
            auditSink.log(.recovery, "pqs.recovery.laneReset outcome=surgicalControlInsert reason=\(reason) peer=\(secretName) deviceId=\(device.deviceId.uuidString) demotedActive=\(demotedActiveCount) consumedOTK=\(sendOneTimeIdentities)")
        }
            ...
            for device in collected {
                ...
                if !existingDeviceIds.contains(device.deviceId), device.deviceId != sessionUser.deviceId {
                    ...
                    if sendOneTimeIdentities,
                       ...
                       let x25519Id = x25519?.id {
                        try await notifyIdentityCreation(
                            for: secretName,
                            x25519Id: x25519Id.uuidString,
            ...
            for foundIdentity in await identities.asyncFilter( { await $0.props(symmetricKey: symmetricKey)?.secretName == secretName }) {
                ...
                for device in verifiedDevices {
                    if currentProps.deviceId == device.deviceId {
                        ...
                        if forceRefresh, !currentProps.hasRatchetState {
                            do {
                                try await attachPublishedPeerOneTimeKeys(
                                    to: &currentProps,
                                    secretName: secretName,
                        ...
                        try await foundIdentity.update(currentProps, symmetricKey: symmetricKey)
    ...
    private func enforcePeerAccountSigningKeyPin(
        ...
        for contact in contacts {
            ...
            if pinnedSigningPublicKey != configuration.signingPublicKey {
                ...
                throw PQSError.peerSigningKeyOutOfSync
            }
        }
    ...
    func createOneTimeKeys(
    ...
    ) async throws -> (x25519: X25519PublicKey?, mlKEM: MLKEMPublicKey){
        ...
        if fetchOneTimeKeys {
            ...
            let consumedX25519Id = keys.x25519?.id.uuidString ?? x25519Id
            let consumedMLKEMId = keys.mlKEM?.id.uuidString ?? mlKEMId
            let fallbackX25519 = try configuration.signedOneTimePublicKeys
                .first(where: { $0.id.uuidString == consumedX25519Id })?
                .verified(using: deviceSigningPublicKey)
            let fallbackMLKEM = try configuration.signedMLKEMOneTimePublicKeys
                .first(where: { $0.id.uuidString == consumedMLKEMId })?
                .verified(using: deviceSigningPublicKey)
            let x25519PublicKey = keys.x25519 ?? fallbackX25519
            let mlKEMPublicKey = keys.mlKEM ?? fallbackMLKEM ?? currentDevice.finalMLKEMPublicKey
            return (x25519PublicKey, mlKEMPublicKey)
        }
        
        let x25519PublicKey = try configuration.signedOneTimePublicKeys.first(where: { $0.id.uuidString == x25519Id })?.verified(using: deviceSigningPublicKey)
    ...
    private func deliverOneTimeIdentityNotifyIfNeeded(
        ...
        guard !props.hasRatchetState else { return }
    ...
    ///   - x25519Id: The initial one-time Curve Key Id associated with the new identity
    ...
    private func notifyIdentityCreation(
        ...
        logger.log(
            level: .info,
            message: "Sent notifyIdentityCreation outbound to nickname(\"\(secretName)\") x25519Id=\(x25519Id)")
    }
