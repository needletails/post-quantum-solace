...
/// communication synchronization. It also provides the concrete implementation for the `ContactService` protocol, allowing the session to
...
public extension PQSSession {
    ...
    ///   - `PQSError.databaseNotInitialized` if the database delegate is not set
    ...
    ///   - `PQSError.drainedKeys` if key refresh fails due to insufficient keys
    ///
    ...
    ///   - `PQSError.sessionNotInitialized` if the session is not properly initialized
    ///   - `PQSError.databaseNotInitialized` if the database delegate is not set
    ///   - `PQSError.invalidSignature` if message signature verification fails
    ///   - `PQSError.drainedKeys` if key refresh fails due to insufficient keys
    ///   - `PQSError.sessionDecryptionError` if message decryption fails
    ///
    ...
    func receiveMessage(
        ...
        do {
            ...
            try await messagePipeline.inboundTask(
                message,
                session: self)
    ...
    func requestMessageResend(
        ...
        markPeerResendRequestSent(
            sender: senderName,
            deviceId: senderDeviceId,
            failedMessageId: sharedMessageIds[0])
        markPeerResendRequestTransported(
            sender: senderName,
            deviceId: senderDeviceId,
    ...
    func handleOutOfBandResendRequest(
        ...
        try await messagePipeline.handleOutOfBandResendRequest(
            from: senderName,
            deviceId: senderDeviceId,
    ...
    func handleOutOfBandResendUnavailable(
        ...
        for messageId in unavailableEnvelopeMessageIds {
            ...
            auditSink.log(.recovery, "pqs.recovery.contentUnrecoverable sharedId=\(messageId) sender=\(senderName) deviceId=\(senderDeviceId.uuidString) reason=oobUnavailable")
    ...
    ///
    /// - Throws:
    ///   - `PQSError.sessionNotInitialized` if the session context is not available
    ///   - `PQSError.databaseNotInitialized` if the cache is not available
    ///   - `PQSError.invalidSignature` if cryptographic operations fail
    ///   - `PQSError.transportNotInitialized` if the transport delegate is not set
    ///
    /// - Important: This method is called internally by `send`. It handles the complex
    ...
    internal func processWrite(
        ...
        try await messagePipeline.outboundTask(
            message: message,
            cache: cache,
    ...
    func createConversation(
        ...
        try await messagePipeline.createChannelCommunication(
            sender: sender,
            recipient: recipient,
    ...
    internal func createChannelCommunication(
        sender: String,
        recipient: MessageRecipient,
        channelName: String,
        administrator: String,
    ...
    func updateChannelMembership(
        ...
        try await messagePipeline.updateChannelMembership(
            channelName: channelName,
            administrator: administrator,
            members: members,
            operators: operators,
...
// MARK: - PQSSession ContactService Protocol Conformance

extension PQSSession: ContactService {
    ...
    private func requireAllSessionParameters() async throws -> (sessionContext: SessionContext,
                                                                ...
                                                                transportDelegate: PQSNetworkHost,
                                                                ...
                                                                symmetricKey: SymmetricKey)
    {
        ...
        guard let receiverDelegate else {
            throw PQSError.receiverDelegateNotSet
        }
